...
import { Create } from '../../browser/create/typings';
...
describe('create-api', () => {
  ...
  async function login(
    credentials: Credentials | undefined = localCredentials() ??
      envCredentials()
  ): Promise<string> {
    if (!credentials) {
      throw new Error('The credentials are not available to log in.');
    }
  ...
  async function cleanAllSketches(): Promise<void> {
    ...
    await Promise.all([
      sketches.map(({ path }) => createApi.deleteSketch(path)),
    ]);
  }
...
function localCredentials(): Credentials | undefined {
  if (!!username && !!password && !!clientSecret) {
    console.log('Using credentials from local variables.');
    return {
      username,
      password,
      clientSecret,
